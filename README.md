...
그런데 만들다 보니 생각이 하나 더 붙었습니다.
**"어차피 만드는 김에, 퀴즈를 조금 더 게임처럼 만들면 나 말고 다른 학우들도
공부를 더 신나게 할 수 있지 않을까?"**
...
황금색(`#d4af37`)과 명조 계열 서체(Noto Serif KR), 어두운 배경과 은은한 글로우로
"고문서를 넘겨보는" 느낌을 노렸습니다. 정답·오답 효과음과 배경음악을 넣어
문제를 푸는 행위 자체가 지루하지 않도록 했습니다.

**혼자 쓰는 요약본이 아니라, 같은 수업을 듣는 학우 누구나 링크 하나로 들어와
재미있게 복습할 수 있는 도구를 만든 것** — 그게 이 프로젝트의 의의입니다.
...
의존성을 하나도 두지 않은 것은 의도한 선택입니다. 수강생 아무나 링크를 열면 바로
동작해야 하고, 학기가 끝난 뒤에도 파일 하나만 남으면 계속 굴러가야 하기 때문입니다.
...
초안을 작성한 뒤 정리한 것**입니다. 단순히 정답만 알려주는 대신 "왜 그 답인지"를 함께 읽게
해서, 문제를 외우는 게 아니라 맥락을 이해하도록 만드는 것이 목적이었습니다.
...
세종대학교 2026학년도 1학기 「과학사」 수강생이 만든 학습용 프로젝트입니다.<br>
공부가 조금이라도 덜 지루해졌다면 성공입니다. ⭐
